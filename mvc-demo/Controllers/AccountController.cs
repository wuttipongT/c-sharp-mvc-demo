        ...
        public ActionResult Login(LoginViewModel model, string returnUrl)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    string pwd = MD5Hash(model.Password);
                    List<se_User> loginInfo = db.se_User.Where(u => u.userName == username && u.userPasswd == pwd).ToList();//(from user in db.se_User where user.userName == model.Username && user.userName == pwd select user).ToList();
                    // Verification.    
